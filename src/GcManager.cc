...
bool GcManager::ForeGC() {
    ...
    //check free segment;
    uint32_t free_seg_num = dataStor_->segMgr_->GetTotalFreeSegs();
    ...
    while (!free_seg_num) {
        ...
        dataStor_->segMgr_->SortSegsByUtils(cands_map, options_.seg_full_rate);
        if (cands_map.empty()) {
            break;
        }
...
void GcManager::BackGC() {
    ...
    uint32_t seg_size = dataStor_->segMgr_->GetSegmentSize();
    uint64_t data_theory_size = idxMgr_->GetDataTheorySize();
    uint32_t theory_seg_num = data_theory_size / seg_size + 1;

    uint32_t free_seg_num = dataStor_->segMgr_->GetTotalFreeSegs();
    uint32_t total_seg_num = dataStor_->segMgr_->GetNumberOfSeg();
    ...
    if (waste_rate > options_.gc_upper_level) {
        while (waste_rate > options_.gc_lower_level && used_seg_num > 1) {
            ...
            used_seg_num = dataStor_->segMgr_->GetTotalUsedSegs();
...
void GcManager::FullGC() {
    ...
    uint32_t free_before = dataStor_->segMgr_->GetTotalFreeSegs();
    uint32_t free_after = 0;
    uint32_t used_total = 0;

    uint32_t seg_num = dataStor_->segMgr_->GetNumberOfSeg();
    ...
    while (true) {
        ...
        if (cands_map.size() < SEG_RESERVED_FOR_GC) {
            free_after = dataStor_->segMgr_->GetTotalFreeSegs();
            used_total = dataStor_->segMgr_->GetNumberOfSeg() - free_after;
        ...
        free_seg_num = dataStor_->segMgr_->GetTotalFreeSegs();
        used_seg_num = seg_num - free_seg_num;
        used_size = (uint64_t) dataStor_->segMgr_->GetSegmentSize()
...
uint32_t GcManager::doMerge(std::multimap<uint32_t, uint32_t> &cands_map) {
    ...
    dataStor_->segMgr_->AllocForGC(seg_first_id);
    ...
    if (!ret) {
        ...
        delete seg_first;
        dataStor_->segMgr_->FreeForFailed(seg_first_id);
    ...
    dataStor_->segMgr_->Use(seg_first_id, free_size);
    ...
    for (std::vector<uint32_t>::iterator iter = free_seg_vec.begin(); iter
        ...
        dataStor_->segMgr_->FreeForGC(*iter);
    }
    delete seg_first;
    cleanKvList(recycle_list);
    ...
    if (!ret) {
        ...
        delete seg_second;
        dataStor_->segMgr_->FreeForFailed(seg_second_id);
        cleanKvList(slice_list);
        return total_free;
    }

    free_size = seg_second->GetFreeSize();
    dataStor_->segMgr_->Use(seg_second_id, free_size);

    seg_second->UpdateToIndex();
    dataStor_->segMgr_->FreeForGC(last_seg_id);
...
bool GcManager::loadKvList(uint32_t seg_id, std::list<KVSlice*> &slice_list) {
    ...
    dataStor_->segMgr_->ComputeSegOffsetFromId(seg_id, seg_phy_off);
