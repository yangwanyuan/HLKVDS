...
class TestMultiTier : public TestNewBase {
public:
    TestMultiTier(): TestNewBase(FILE_PATH, 1) {}
};

TEST_F(TestMultiTier, OpenDB) {
    KVDS *db = Create();
    EXPECT_TRUE( NULL != db);
    delete db;
...
TEST_F(TestMultiTier, WriteBatch) {
    KVDS *db = Create();
...
TEST_F(TestMultiTier, InsertImmediately) {
    KVDS *db = Create();
